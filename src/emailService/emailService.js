...
export async function sendPaymentNotification(userData, image_url, nama_bank, nama_di_rekening, nomor_rekening, kode_promo, items) {
     //Get today's date for payment date
     var today = new Date(); //Date of payment
     var date = today.getDate().toString();
     var month = today.getMonth() < 10 ? '0' + (today.getMonth() + 1).toString() : today.getMonth().toString() + 1;
    ...
    var total_price = 0;
    items.forEach((item) => {
        items_string = items_string + item.book_title + "        " + item.price + "<br>";
